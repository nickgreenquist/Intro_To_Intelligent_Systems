...
# Usage: statToCsv.py resultDirectory outputFile
...
USAGE_MESSAGE = "Usage: statToCsv.py resultDirectory outputFile"
...
def processFile(fileName: str) -> (str, float):
...
def main():
	...
	for test in TESTS: # Read results into dictionary
		...
		for i in range(0, len(os.listdir(testDir))):
			dataset, score = processFile(os.path.join(testDir, "result{}s0.stat".format(i)))
